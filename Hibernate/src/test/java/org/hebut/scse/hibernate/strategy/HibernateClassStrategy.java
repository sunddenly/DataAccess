package org.hebut.scse.hibernate.strategy;

import org.hebut.scse.hibernate.HibernateUtil;
import org.hebut.scse.hibernate.pojo.strategy.Customer;
import org.hebut.scse.hibernate.pojo.strategy.Order;
...
public class HibernateClassStrategy {
    ...
    //        System.out.println(customer.getOrders().size());//不会导致增强延迟加载的初始化
    //        Order order = new Order();
    //        order.setOrderId(1);
    //        System.out.println(customer.getOrders().contains(order));//不会导致增强延迟加载的初始化
    //
        Hibernate.initialize(customer.getOrders());//初始化代理对象
//
//        //---------------set 的 lazy 属性------------------
...
//        //3. lazy 还可以设置为 extra. 增强的延迟检索. 该取值会尽可能的延迟集合初始化的时机!
    ...
    public void testClassLevelStrategy(){
        ...
        System.out.println(customer.getClass());//Customer_$$_javassist_1

        System.out.println(customer.getCustomerId());//不生成sql
        System.out.println(customer.getCustomerName());//生成sql
    }
